...
public class MeeleAttackBGObjects : MonoBehaviour
{
	...
	public static void Action(Vector2 point, float radius, int layerMask, float damage, bool allTargets)
	{
		...
		if(!allTargets)
		{
			...
			if(obj != null && obj.GetComponent<BgObjectHP>())
			{
				obj.GetComponent<BgObjectHP>().HP -= damage;
			}
			return;
		}
		...
		{
			if(hit.GetComponent<BgObjectHP>())
			{
				hit.GetComponent<BgObjectHP>().HP -= damage;
			}
		}
	}
}
